  ...
  return (
    <div>
      <div className="row" style={{ marginBottom: '1rem' }}>
        ...
        <button className={`btn btn-sm ${tab === 'privacy' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => setTab('privacy')}>Privacy & safety</button>
      </div>
      {tab === 'profile' ? <Profile /> : tab === 'certs' ? <Certificates /> : tab === 'referrals' ? <Referrals /> : tab === 'privacy' ? <PrivacySafety /> : <Tools />}
    </div>
  );
}
